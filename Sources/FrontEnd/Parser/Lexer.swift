...
public struct Lexer: IteratorProtocol, Sequence {
  ...
  public mutating func next() -> Token? {
    /// Are there pre-fetched tokens?
    if let t = prefetched.popLast() { return t }

    // Do we have to process indentation?
    while isAtLineStart {
    ...
    if head.isNewline {
      discard()
      discard(while: { (c) in c.isWhitespace && !c.isNewline })
    }
  ...
  /// Discards all whitespaces and comments preceding the next token or newline, returning an error
  /// token iff the scanner encounters an unmatched block comment opening sequence.
